 ...
 */
  ...
  */
  downLoadDetaiilImgs: function () {
    var that = this
    id = 'osaod0ZlCZxFk3qxoDRrrx9lRvU9'
    wx.request({
      url: api.devDomain + 'getHomeDetail?id=' + id,
      method: 'GET',
      success: function (res) {
        console.log(res)
        if (200 == res.data.code) {
          var message = JSON.parse(res.data.result);
          res.data.result = JSON.parse(message);
          if (null != res.data.result && res.data.result.length >= 1) {
            for (var i = 0; i < res.data.result.length;i++){
              imgUrlsDefaultPre[i] = res.data.result[i].imgUrl
            }
          }
          //更新数据
          that.setData({
